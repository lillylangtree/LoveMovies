angular.module('movieDBControllers', [])
    ...
    .controller('MovieFavoritesController', function ($scope, MovieListService, $location, $routeParams) {
        $scope.loading = true;
        ...
        $scope.deleteFavorite = function(movie) {
            MovieListService.deleteFavorite(movie).then(
                function (result) {
...
// 
        ...
        $scope.addFavourite = function(){
            MovieListService.postFavorite($scope.movie).then(
                function (result) {
                    if (result.status == 200 && result.statusText == 'OK') {
                        $scope.showFav = false;
                    }
